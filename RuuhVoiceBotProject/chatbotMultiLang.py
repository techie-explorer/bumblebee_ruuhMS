...
tts = gTTS('hello' + user.name, lang="en-GB", slow=False)
tts.save("hello.mp3")
playsound("hello.mp3")
...
def ruuh():
    ...
    if "secure my drive" in result:
        try:
            os.system("python drowsiness.py  --shape-predictor shape_predictor_68_face_landmarks.dat --alarm alarm.wav")
        except Exception as e:
            print("Error Occured", e)
            start_porcupine()
    if "सेल्फी" in result_temp:
        os.system("python capture.py")
        client.sendLocalFiles("opencv5.png", Message(
            "Hello"), thread_id="118809975300669", thread_type=ThreadType.USER)
        flag = 1
    if (flag == 0):
    ...
    if(flag == 1):
        messages = client.fetchThreadMessages(
            thread_id="118809975300669", limit=1)
    else:
        messages = client.fetchThreadMessages(
            thread_id="118809975300669", limit=2)
    messages.reverse()
    ruuh_reply = ""
